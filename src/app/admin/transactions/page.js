        ...
        <div className="lg:col-span-1 border border-gray-800 bg-gray-900 rounded-xl shadow-sm overflow-hidden sticky top-8 h-fit">
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleTransaction} className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">Select Client</label>
                <select
                  required
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-2">
                  <label className={`cursor-pointer rounded-lg border p-3 flex items-center justify-center font-medium transition-colors ${type === 'INVEST' ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400' : 'border-gray-700 bg-gray-950 text-gray-400 hover:bg-gray-800'}`}>
                    <input type="radio" value="INVEST" checked={type === 'INVEST'} onChange={() => setType('INVEST')} className="sr-only" />
                    Investment
                  </label>
                  <label className={`cursor-pointer rounded-lg border p-3 flex items-center justify-center font-medium transition-colors ${type === 'WITHDRAW' ? 'border-red-500 bg-red-500/10 text-red-400' : 'border-gray-700 bg-gray-950 text-gray-400 hover:bg-gray-800'}`}>
              ...
              <div>
                ...
                {amount && (
                  <p className="text-xs text-gray-400 mt-2">
                    Estimated Layout: <span className="text-white font-medium">~{(Number(amount)/manualNav).toFixed(4)} Units</span>
                  </p>
                )}
              </div>
                  ...
                  {transactions.map((t) => (
                    <tr key={t.id} className="border-b border-gray-800 hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-gray-300 font-medium">{t.client?.name}</div>
                      </td>
                      ...
                      <td className={`px-6 py-4 text-right font-semibold ${t.type === 'INVEST' ? 'text-emerald-400' : 'text-red-400'}`}>
